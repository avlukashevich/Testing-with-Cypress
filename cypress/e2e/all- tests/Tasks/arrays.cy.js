...
context('Learning arrays', () => {
	it('TESTS', () => {
		...
		let newPlanetsArray = planetsArray.map(planet => {
			return Object.assign(planet, { solarSystem: null })
		})
		...
		let radiusSum = planetsArray.reduce((total, planet) => total + planet.radius, 0);
		...
		let index = planetsArray.map(i => i.planet).indexOf('SomeNewPlanet');
		...
		let radiuses = planetsArray.map(planet => planet.radius);
		radiuses.sort(function (a, b) { return a - b });
		cy.log('===Сортировка планет по радиусу===');
		printPlanets(radiuses);

		//Задание 8
		//Сортировка планет по имени
		let names = planetsArray.map(i => i.planet);
